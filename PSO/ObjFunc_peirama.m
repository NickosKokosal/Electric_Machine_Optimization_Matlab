...
Thickmagnet= X(5);      % magnet thickness
...
P       = 3*E*I;
% Test efficiency
Spp     = 1; % Number of slots per pole per phase
RhoCu   = 1.725*(10^(-8)); % Resistivity of copper
miron   = 7860; % Mass density of iron
Po      = 5.637; % Iron losses per volume @ 50Hz, 1.5T
% Mass
...
PfyokeS = 1.5*Po*(((Nn/(50*60))^1.5)*((IndyokeS/1.5)^2.2))*MyokeS; %Iron Losses Stator Yoke 
PfyokeR = 1.5*Po*(((Nn/(50*60))^1.5)*((IndyokeR/1.5)^2.2))*MyokeR; %Iron Losses Rotor Yoke 
Piron   = Pftooth+PfyokeS+PfyokeR;
% Joules losses
...
alpha12 = (M/0.67)-0.49253;
mu      = (eff/0.0241)-39.41;
Mnew    = (sqrt(M)/0.426349)- 1.3454961;
% opti   = 1/(alpha12*eff+mu*M);
opti    = 1/(0.6*effnew+0.4*Mnew);
%opti    = 1/(0.7*effnew+0.3*Mnew);
